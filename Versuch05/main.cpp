...
int main()
{
	...
	do
	{
		std::cout << "\nMenue:" << std::endl
				...
				<< "(5): Datenelement nach Matrikelnummer loeschen"<< std::endl
		...
		// Datenelement nach Matrikelnummer loeschen
		case '5':
		{
			if (!studentenListe.empty())
			{
				unsigned int matNr;
				std::cout
						<< "Bitte geben Sie die Matrikelnummer des zu loeschenden Studenten ein: ";
				std::cin >> matNr;
				std::cin.ignore(10, '\n');
				...
				while (!studentenListe.empty())
				{
					...
					{
						std::cout
								<< "Der folgende Student ist geloescht worden:"
								<< std::endl;
						student.ausgabe();
						found = true;
					}
					...
					{
						tempListe.pushBack(student);
					}
				}
				studentenListe = tempListe;

				if (!found)
				{
					std::cout
							<< "Es wurde kein Student mit dieser Matrikelnummer gefunden.\n";
				}
			}
			else
			{
